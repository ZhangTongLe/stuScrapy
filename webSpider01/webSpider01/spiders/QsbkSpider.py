#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2017/8/9 14:21
# @Author  : fengye
# @Site    : 
# @File    : QsbkSpider.py
# @Software: PyCharm

import scrapy
from scrapy.http import Request
from webSpider01.items import QsbkItem
import sys

reload(sys)
sys.setdefaultencoding('utf8')
class QsbkSpider(scrapy.Spider):

    name = "qsbk_spider"
    start_urls = [
        "https://www.qiushibaike.com/"
    ]

    # 模拟浏览器 要不然会报错
    user_agent = 'Mozilla/4.0 (compatible; MSIE 5.5; Windows NT)'
    headers = {
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
        "Accept-Encoding": "gzip, deflate, sdch",
        "Accept-Language": "zh-CN,zh;q=0.8",
        "Cache-Control": "max-age=0",
        "Cookie": "CAMPUS_CHECKED=0; JYERN=0.6521779221948236; CAMPUS_CHECKED=0; QS_ED=6; JYERN=0.26531747193075716; QS_GD=7; QS_TM=1; jyean=ZnkIgT1JW7dtwnfPBYUOaQMQ9EHw9Hrhs72JCf4tOTiBEDLIHPZF72BzqUmmVHNWAvt1xk4aizTajtIpfTvTrEE39lecGsS2bVqdUtacbOjnYA2_qVZSih0wl0Ao7Xe00; LF_Email=njxczx08@xyh.com; jy=; jye_notice_show=1|2015/8/31 15:08:32|0|false; JYERN=0.39406700897961855; CAMPUS_CHECKED=0; jye_math_ques_s=0; jye_math_ques_d=0; jye_math_ques_t=1; jye_math_ques_0_q=070eac9c-ad95-44f1-909c-869dbb4c874a~803641a7-c36b-483b-9f8d-ac0a5522e3c3~; CNZZDATA2018550=cnzz_eid%3D484121901-1440586726-http%253A%252F%252Fwww.baidu.com%252F%26ntime%3D1441002725; _cnzz_CV2018550=%E7%94%A8%E6%88%B7%E4%BB%98%E8%B4%B9%7C%E9%9D%9EVIP%E7%94%A8%E6%88%B7%7C1441006330746%26%E7%94%A8%E6%88%B7%E8%A7%92%E8%89%B2%7C%E8%80%81%E5%B8%88%7C14",
        "Host": "www.qiushibaike.com",
        "Proxy-Connection": "keep-alive",
        "Referer": "http://www.qiushibaike.com/",
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/40.0.2214.111 Chrome/40.0.2214.111 Safari/537.36"
    }

    def start_requests(self):
        for url in self.start_urls:
            yield self.make_requests_from_url(url)

    def make_requests_from_url(self, url):
        return Request(url, headers=self.headers, dont_filter=True)

    def parse(self, response):

        #解析抓取到的内容
        item = QsbkItem()
        '''
        这里查看网页源代码的时候发现 每一条糗百样式都不太一样，有
       //div[@class="article block untagged mb15 typs_recent"]
       //div[@class="article block untagged mb15 typs_hot"]
       //div[@class="article block untagged mb15 typs_old"]
       //div[@class="article block untagged mb15 typs_long"]
        在这里只是爬取了一种类型
        '''
        for article in response.xpath('//div[@class="article block untagged mb15 typs_recent"]'):
            try:
                #作者
                author = article.xpath('div[@class="author clearfix"]/a/h2/text()').extract_first()
                #作者头像
                author_logo = article.xpath('div[@class="author clearfix"]/a/img/@src').extract_first()
                #内容
                content = article.xpath('a[@class="contentHerf"]/div[@class="content"]/span/text()').extract_first()

                #图片
                thumb = article.xpath('div[@class="thumb"]/a/img/@src').extract_first()
                #好笑
                votes = article.xpath('div[@class="stats"]/span[@class="stats-vote"]/i/text()').extract_first()

                # 评论数
                comments = article.xpath('div[@class="stats"]/span[@class="stats-comments"]/a/i/text()').extract_first()

                item['author'] = author.encode("utf-8")
                item['author_logo'] = author_logo.encode('utf-8')
                item['content'] = content.encode('utf-8')
                item['thumb'] = thumb.encode('utf-8')
                item['votes'] = votes
                item['comments'] = comments
                print "===================================="*3
                print "author       :", item['author']
                print "author_logo  :", item['author_logo']
                print "content      :", item['content']
                print "thumb        :", item['thumb']
                print "vote         :", item['votes']
                print "comments      :", item['comments']
                print "\r\n"

                # 关键字yield 防止一次生成过多的字典占用大量的内存
                yield item
            except Exception,e:
                print "get info error! continue get next info!"
                print e.message
                continue
        #获取下一页的链接
        pages = response.xpath('//ul[@class="pagination"]/li')
        next_page = pages[-1].xpath('a/@href').extract()[0]
        #如果已经时最后一页了，在继续就到更多内容了，这里我们只抓取首页的内容
        if next_page == '/week/':
            print "====over===="
            return
        else:
            next_page_url = "https://www.qiushibaike.com" + next_page
            print next_page_url
            yield Request(url=next_page_url, headers=self.headers, callback=self.parse)


